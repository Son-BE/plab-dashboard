...
(function(){
  ...
  var CONFIG = {
    API_URL: 'https://script.google.com/macros/s/AKfycbyzION97yz6ThNASZ8PRhjv8J8dGPuzxQH1Md0g6AQbkn6P72ff90IuaadBQcPLR0TpKg/exec',
  ...
  var STATE = { rows: [], lastSync: null, lastDigest: null, sessionCount: 6 };
  ...
  var appRoot, statsGrid, rowsBody, tableHeadEl, rowCountLabel, searchInput, regionNavEl, roundNavEl, stepMessageEl, filterStatus, filterFlag,
      ...
      stepOpenDateSaveBtn, stepOpenDateCancelBtn;
  ...
  // 자동으로 매겨요: 3일↑ 주의, 5일↑ 경고, 7일↑ 문제. 이미 완료된 기업이거나 협약일자가
  // 없어서 계산할 수 없는 기업은 정상으로 둬요 (협약일자 없음은 비고란에 따로 안내해요).
  ...
  function normalizeRow(r){
    ...
    var normalized = {
      ...
      complete: complete,
      flag: computeAutoFlag(dateVal, complete), remark: r.remark || '', round: r.round || DEFAULT_ROUND,
  ...
  // 이슈 배지는 이제 관리자든 협력사든 직접 바꿀 수 없는 자동 계산 결과라서, 항상 읽기 전용
  // 배지로만 보여줘요. 마우스를 올리면 왜 이 상태인지(며칠 지났는지) 알 수 있게 안내를 달아요.
  ...
  function flagCellHTML(r){
    ...
    return '<span class="flag-badge flag-' + flag + '" title="' + escapeHtml(flagTooltip(r)) + '">' + escapeHtml(label) + '</span>';
  }
  ...
  function renderUploadRowHTML(r){
    ...
    return '' +
    '<tr data-id="' + r.id + '">' +
      '<td><span class="readonly-text">' + escapeHtml(r.date || '') + '</span></td>' +
      '<td><span class="readonly-text">' + escapeHtml(r.folderNc || '') + '</span></td>' +
      '<td><span class="readonly-text">' + escapeHtml(r.region || '') + '</span></td>' +
      '<td class="sticky-col company-cell"><span class="readonly-text">' + escapeHtml(r.company || '') + '</span></td>' +
      items.map(function(it){ return '<td class="center">' + uploadToggleHTML(r.id, it.field, it.checked, it.available) + '</td>'; }).join('') +
      '<td class="center">' + autoCellHTML(uploadComplete, true) + '</td>' +
    '</tr>';
  }
  ...
  function renderStats(){
    ...
    statsGrid.innerHTML =
      ...
      '<div class="stat-card"><div class="stat-label">이슈 표시</div><div class="stat-value">' + s.issueCount + '<span class="stat-unit">문제</span></div>' +
        '<div class="stat-value small">' + s.alertCount + '<span class="stat-unit">경고</span> · ' + s.cautionCount + '<span class="stat-unit">주의</span></div></div>' +
  ...
  function applyRoleUI(){
    ...
    if(tip){
      if(uploadGated){
        ...
        tip.hidden = false;
        tip.textContent = '파란 테두리 칸은 취합현황에 이미 자료가 올라온 회차·서류예요. 외부 사이트에 업로드를 마쳤으면 체크해주세요 — 관리자와 협력사 계정 모두 체크할 수 있어요. 색깔 없는 칸(취합현황에 자료가 아직 없는 항목)은 체크할 수 없어요.';
      } else {
        tip.hidden = !admin;
        tip.textContent = '팁: 1~' + STATE.sessionCount + '회차 · 계획서 · 보고서 · 완료는 "드라이브에서 불러오기"를 눌렀을 때 멘티기업 폴더 안 파일 유무를 확인해서 자동으로만 체크돼요 (직접 클릭해서 바꿀 수 없어요). 이슈 표시(주의/경고/문제)는 협약일자로부터 지난 일수(3일/5일/7일)를 기준으로 자동 계산되고, 비고는 관리자가 직접 입력할 수 있어요.';
      }
    }
  ...
  function onTableChange(e){
    ...
    if(field === 'date'){
      ...
      row.flag = computeAutoFlag(row.date, row.complete);
      tr.className = 'flag-' + row.flag;
      var flagBadge = tr.querySelector('.flag-badge');
      if(flagBadge){
        flagBadge.className = 'flag-badge flag-' + row.flag;
        flagBadge.title = flagTooltip(row);
        flagBadge.textContent = FLAG_LABELS[row.flag] || '정상';
      }
  ...
  function bindEvents(){
    ...
    searchInput.addEventListener('input', function(){
      ...
      renderRegionNav(); renderRoundNav(); renderTable(); renderRowCountLabel();
    });
    ...
    if(sessionSettingsBtn) sessionSettingsBtn.addEventListener('click', function(){
      ...
      if(stepOpenDateBanner) stepOpenDateBanner.hidden = true;
    });
    if(stepOpenDateBtn) stepOpenDateBtn.addEventListener('click', function(){
      ...
      if(sessionCountBanner) sessionCountBanner.hidden = true;
    });
  ...
  function init(){
    ...
    if(isPlaceholder(apiUrl)){
      // 아직 API_URL을 설정하지 않은 로컬 미리보기 상태예요. 로그인 없이 관리자
      // 화면 예시를 그대로 보여줘요 (실제 배포에서는 이 배너가 뜨지 않아요).
